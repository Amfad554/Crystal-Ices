      ...
      try {
        const response = await fetch(
          "http://localhost:5000/api/users/verifyemail",
          {
            ...
            body: JSON.stringify({ token: token }), // 'token' here comes from useParams()
          }
        );
        ...
        {status === "error" && (
          <div className="space-y-4">
            ...
            <p className="text-slate-600">
              The token is expired or the backend is unreachable.
            </p>
            <button
              onClick={() => navigate("/auth")}
              className="mt-6 w-full bg-[#0B2A4A] text-white py-4 rounded-xl font-bold uppercase tracking-widest text-[11px]"
            >
              Back to Login
            </button>
          </div>
